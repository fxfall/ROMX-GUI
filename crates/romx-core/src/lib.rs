...
impl Footer {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, RomxError> {
        if bytes.len() != FOOTER_SIZE {
            return Err(RomxError::Invalid("footer must be exactly 128 bytes".into()));
        }
        if &bytes[0..4] != MAGIC {
        ...
        if version != VERSION || footer_size != FOOTER_SIZE as u32 {
            return Err(RomxError::Invalid("unsupported version or footer_size".into()));
        }
        ...
        Ok(Self {
            version,
            rom: Region { offset: read_u64(bytes, 0x08), size: read_u64(bytes, 0x10) },
            metadata: Region { offset: read_u64(bytes, 0x18), size: read_u64(bytes, 0x20) },
            cover: Region { offset: read_u64(bytes, 0x28), size: read_u64(bytes, 0x30) },
...
fn validate_metadata(metadata: &Value) -> Result<(), RomxError> {
    let object = metadata
        .as_object()
    ...
    for key in ["schema_version", "label", "platform", "payload_format"] {
        if !object.contains_key(key) {
            return Err(RomxError::Invalid(format!("metadata missing required field: {key}")));
        }
    }
    if object.get("schema_version") != Some(&Value::String("1.0".into())) {
        return Err(RomxError::Invalid("metadata schema_version must be 1.0".into()));
    }
    let label = object.get("label").and_then(Value::as_str).unwrap_or("");
    if label.is_empty() {
        return Err(RomxError::Invalid("metadata label must not be empty".into()));
    }
    let platforms = ["gb", "gbc", "gba", "nes", "snes", "nds", "3ds", "genesis"];
    let formats = ["gb", "gbc", "gba", "nes", "fds", "sfc", "smc", "nds", "3ds", "cci", "cia", "md", "gen", "smd", "bin"];
    ...
    if !formats.contains(&object.get("payload_format").and_then(Value::as_str).unwrap_or("")) {
        return Err(RomxError::Invalid("unsupported metadata payload_format".into()));
    }
    if let Some(cover) = object.get("cover") {
        if cover.get("mime_type").and_then(Value::as_str) != Some("image/png") {
            return Err(RomxError::Invalid("cover.mime_type must be image/png".into()));
        }
    }
    Ok(())
}
...
fn validate_regions(footer: &Footer, body_len: usize) -> Result<(), RomxError> {
    ...
    let regions = [("rom", footer.rom), ("metadata", footer.metadata), ("cover", footer.cover)];
    ...
    for (index, first) in nonempty.iter().enumerate() {
        for second in nonempty.iter().skip(index + 1) {
            if first.1 < second.2 && second.1 < first.2 {
                return Err(RomxError::Invalid(format!("regions overlap: {} and {}", first.0, second.0)));
            }
        }
    }
    ...
    {
        return Err(RomxError::Invalid("footer flags do not match region sizes".into()));
    }
    Ok(())
}

pub fn pack_bytes(rom: &[u8], metadata: Option<&Value>, cover: Option<&[u8]>) -> Result<Vec<u8>, RomxError> {
    if rom.is_empty() {
        return Err(RomxError::Invalid("ROM payload must not be empty".into()));
    }
    ...
    let rom_region = Region { offset: 0, size: rom.len() as u64 };
    let metadata_region = Region { offset: metadata_bytes.as_ref().map(|_| rom.len() as u64).unwrap_or(0), size: metadata_bytes.as_ref().map(|v| v.len() as u64).unwrap_or(0) };
    ...
    let cover_region = Region { offset: cover_offset, size: cover.map(|v| v.len() as u64).unwrap_or(0) };
    let mut body = Vec::with_capacity(rom.len() + metadata_region.size as usize + cover_region.size as usize);
    body.extend_from_slice(rom);
    if let Some(bytes) = &metadata_bytes { body.extend_from_slice(bytes); }
    if let Some(bytes) = cover { body.extend_from_slice(bytes); }
    let mut flags = FLAG_BODY_SHA256;
    if metadata_region.size > 0 { flags |= FLAG_METADATA; }
    if cover_region.size > 0 { flags |= FLAG_COVER; }
    let footer = Footer { version: VERSION, rom: rom_region, metadata: metadata_region, cover: cover_region, rom_sha256: sha256(rom), flags, body_sha256: sha256(&body) };
    body.extend_from_slice(&footer.encode());
    Ok(body)
}

pub fn pack_to_path(rom_path: &Path, metadata_path: Option<&Path>, cover_path: Option<&Path>, output_path: &Path) -> Result<(), RomxError> {
    let rom = fs::read(rom_path)?;
    let metadata = if let Some(path) = metadata_path { Some(serde_json::from_slice::<Value>(&fs::read(path)?)?) } else { None };
    let cover = if let Some(path) = cover_path { Some(fs::read(path)?) } else { None };
...
pub fn read_bytes(bytes: &[u8]) -> Result<RomxDocument, RomxError> {
    if bytes.len() < FOOTER_SIZE { return Err(RomxError::Invalid("file is shorter than footer".into())); }
    ...
    if sha256(&rom) != footer.rom_sha256 { return Err(RomxError::Invalid("ROM SHA-256 mismatch".into())); }
    if footer.flags & FLAG_BODY_SHA256 != 0 && sha256(body) != footer.body_sha256 { return Err(RomxError::Invalid("body SHA-256 mismatch".into())); }
    let metadata = if footer.metadata.size > 0 {
        let value: Value = serde_json::from_slice(slice_region(body, "metadata", footer.metadata)?)?;
        validate_metadata(&value)?;
        Some(value)
    } else { None };
    let cover = if footer.cover.size > 0 {
        let value = slice_region(body, "cover", footer.cover)?;
        if !value.starts_with(PNG_SIGNATURE) { return Err(RomxError::Invalid("embedded cover is not PNG".into())); }
        Some(value.to_vec())
    } else { None };
    Ok(RomxDocument { footer, rom, metadata, cover })
}

pub fn read_path(path: &Path) -> Result<RomxDocument, RomxError> { Ok(read_bytes(&fs::read(path)?)?) }

pub fn extract_to_dir(path: &Path, output_dir: &Path) -> Result<PathBuf, RomxError> {
    let document = read_path(path)?;
    fs::create_dir_all(output_dir)?;
    let format = document.metadata.as_ref().and_then(|v| v.get("payload_format")).and_then(Value::as_str).unwrap_or("rom");
    let payload_path = output_dir.join(format!("payload.{format}"));
    fs::write(&payload_path, &document.rom)?;
    if let Some(metadata) = document.metadata { fs::write(output_dir.join("metadata.json"), serde_json::to_vec_pretty(&metadata)?)?; }
    if let Some(cover) = document.cover { fs::write(output_dir.join("cover.png"), cover)?; }
    Ok(payload_path)
}
...
pub fn required_metadata(label: impl Into<String>, platform: impl Into<String>, payload_format: impl Into<String>) -> Value {
    ...
    object.insert("payload_format".into(), Value::String(payload_format.into()));
    Value::Object(object)
}
